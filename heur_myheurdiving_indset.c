 ...
 * @author Your Name
 */
#include "scip/heur_myheurdiving_indset.h"
...
#define HEUR_NAME             "indset_diving"
...
)
{
   ...
   if( mayrounddown )
      s -= 3.0;
   if( mayroundup )
      s -= (-3.0);  /* 注意：Python 中是 "- -3.0"，即 +3.0 */

   /* 结合分数部分和目标系数 */
   if( mayrounddown )
      s += (1.0 - candsfrac) * obj * 0.5;
   if( mayroundup )
      s += candsfrac * obj * 0.5;

   /* 加入伪成本调整 */
   if( mayrounddown )
      s += pscostdown * candsfrac * 1.5;
   if( mayroundup )
      s += pscostup * (1.0 - candsfrac) * 1.5;

   /* 对偏离根节点解施加较轻的惩罚 */
   s -= fabs(rootsolval - candsol) * 0.1;

   /* 归一化分数（若 objnorm > 0） */
   if( objnorm > 0.0 )
      s /= objnorm;

   /* 对二元变量给予额外奖励，并调整锁计数影响 */
   s += (SCIP_Real)nlocksup * 0.3 - (SCIP_Real)nlocksdown * 0.3;
   if( isBinary )
      s += 1.0;
...
SCIP_DECL_HEURCOPY(heurCopyMyheurdiving)
{
   ...
   SCIP_CALL( SCIPincludeHeurMyheurdiving_indset(scip) );
   return SCIP_OKAY;
}
 ...
 */
SCIP_RETCODE SCIPincludeHeurMyheurdiving_indset(SCIP* scip)
{
   SCIP_HEURDATA* heurdata;
   SCIP_HEUR* heur;
