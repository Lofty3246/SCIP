    ...
    with open(output_file, 'w') as f:
        ...
        while True:
            try:
                ...
                f.write(f"{timestamp},{cpu_percent},{mem_percent},{psutil.virtual_memory().used/1024**3},{psutil.virtual_memory().total/1024**3}\n")
                f.flush()  # 确保实时写入文件
                time.sleep(interval)
...
if __name__ == '__main__':
    monitor_file = "resource_usage.csv"
    monitor_process = multiprocessing.Process(target=monitor_resources, args=(monitor_file,))
    monitor_process.start()
    parser = argparse.ArgumentParser()
    parser.add_argument(
        'problem',
        help='MILP instance type to process.',
        choices=['setcover', 'cauctions', 'facilities', 'indset','setcover_new'],
    )
    parser.add_argument(
        '-s', '--seed',
    ...
    if args.problem == 'setcover':
    ...
    elif args.problem == 'setcover_new':
        nrows = 500
        ncols = 1000
        dens = 0.05
        max_coef = 100
        ...
        for i in range(5):
            ...
            lp_dir = f'../data/instances/setcover_new/train_{i}'
            print(f"{n} instances in {lp_dir}")
            os.makedirs(lp_dir)
            filenames.extend([os.path.join(lp_dir, f'instance_{i+1}.lp') for i in range(n)])
            nrowss.extend([nrows] * n)
