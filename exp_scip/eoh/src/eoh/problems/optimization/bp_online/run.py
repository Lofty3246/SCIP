...
class BPONLINE():
    ...
    def get_valid_bin_indices(self,item: float, bins: np.ndarray) -> np.ndarray:
        ...
        # np.nonzero() 是 NumPy 中的一个函数，用于返回数组中非零元素的索引
    ...
    def evaluateGreedy(self,alg) -> float:
        ...
        for name, dataset in self.instances.items():
            num_bins_list = []
            i = 0
            for iter, instance in dataset.items():
                ...
                # fitness = (num_bins- self.lb[name][i]) / self.lb[name][i]
                # i += 1
                # print(f"Instance: {iter}, Fitness: {fitness}")
                # score_list = []
                # score_list.append(fitness)
            ...
            # fitness = np.mean(np.array(score_list))
    ...
    def evaluate(self, code_string):
        try:
            ...
            print(f"this is Evaluation error: {e}")  # 打印具体错误
            import traceback
