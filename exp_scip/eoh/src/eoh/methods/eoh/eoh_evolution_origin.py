...
class Evolution():
    ...
    # get_prompt func  提取prompt内容并return
...
+self.prompt_other_inf+"\n"+"Do not give additional explanations."
    ...
    # 除了inital,其他的都需要将选取的父代[code,algorithm]作为content传入
...
+self.prompt_inout_inf+"\n"+"Do not give additional explanations."
    ...
    # 从llm获取response,return[code，algorithm]
    def _get_alg(self,prompt_content):
        ...
        while (len(algorithm) == 0 or len(code) == 0):
            ...
            print("code generate fail,try again\n")
            response = self.interface_llm.get_response(prompt_content)
            ...
            # 只匹配从 import 到 def score 的代码，跳过中间的自然语言
            pattern = r"import[^\n]*(?:\n\s*[^\n]*)*?\n\s*def\s+score\s*$$[^)]*$$\s*->[^:]*:.*?return\s*"

            code = re.findall(pattern, response, re.DOTALL)

            if not code:
                # 备用：尝试只匹配 def score 函数
                code = re.findall(r"def\s+score\s*$$[^)]*$$\s*->[^:]*:.*?return\s*", response, re.DOTALL | re.MULTILINE)
    ...
    # operator 调用prompt_content，获得prompt，调用_get_alg(prompt_content)，获得[code，algorithm]
    def i1(self):
